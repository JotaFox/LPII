public class Ex1 {
	...
	public static void main(String[] args) {
		...
		if (nArgs == 6) {
			...
			try {
				...
				double distAB = Math.sqrt(((xB - xA) * (xB - xA))
						+ ((yB - yA) * (yB - yA)));
				double distAC = Math.sqrt(((xC - xA) * (xC - xA))
						+ ((yC - yA) * (yC - yA)));
				double distBC = Math.sqrt(((xC - xB) * (xC - xB))
						+ ((yC - yB) * (yC - yB)));
				...
				for (int i = 0; i < nArgs; ++i) {
					try {
						...
						System.out.println("Problema no " + (i + 1)
								+ "o argumento: " + args[i]);
					}
				}
			}
	...
	public static double calculaPerimetro(double distAB, double distAC,
			double distBC) {
	...
	public static double calculaArea(double distAB, double distAC, double distBC) {
		...
		double area = Math.sqrt((semiperimetro * (semiperimetro - distAB)
				* (semiperimetro - distAC) * (semiperimetro - distBC)));
		return area;
	}

	public static void imprimeResultados(double distAB, double distAC,
			double distBC) {
	...
	public static void processaTriangulo(double distAB, double distAC,
			double distBC) {

		if ((distAB < distAC + distBC) && (distAC < distAB + distBC)
				&& (distBC < distAB + distAC)) {// verifica se é um triangulo
			...
			if (distAB == distAC && distAB == distBC) {
				...
				System.out
						.println("Três lados iguais . Trata-se de um Triangulo Equilatero.\n");
			} else if ((distAB == distAC) || (distAB == distBC)) {

				System.out
						.println("Dois lados iguais . Trata-se de um Triangulo Isosceles.\n");
			} else {

				System.out
						.println("Três lados diferentes. Trata-se de um Triangulo Escaleno.\n");
			}
		} else {
